...
def print_catalogue_summary(df: pl.DataFrame, title: str):
    ...
    both_count = df.filter(pl.col("SAFE SLC").is_not_null() & pl.col("SAFE GRD").is_not_null()).height
    ...
    if "dataset(s) d'appartenance" in df.columns:
        ...
        if exploded.height > 0:
            dataset_counts = exploded.group_by("dataset(s) d'appartenance").agg(pl.len())
            print("\nDataset memberships:")
...
def main():
    ...
    if "dataset(s) d'appartenance" in df2.columns:
        ...
        if lion_products.height > 0:
            print("  First few:")
            print(lion_products.select(["SAFE GRD", "dataset(s) d'appartenance"]).head(3))
    
    # Check that existing rows preserved their data
    # Compare start date of a known product
    known_grd = "S1A_IW_GRDH_1SDV_20190711T181228_20190711T181253_028073_032B9D_6E14.SAFE"
    ...
    print(f"horodating range - updated: {df2['horodating'].min()} to {df2['horodating'].max()}")
