...
def sample_catalogue_df():
    data = {
        ...
        "SAFE OCN": [None],
        "presence SLC": ["/path/to/slc"],
        "presence GRD": ["/path/to/grd"],
        "presence OCN": [None],
        "presence L1B XSP A21": [None],
        "presence L1C XSP B17": [None],
        "dataset(s) d'appartenance": [["test_slc", "test_grd"]],
...
class TestBuildFromListings:
    ...
    def test_build_from_listings_single_slc(self, tmp_path, updater):
        ...
        # assert df["dataset(s) d'appartenance"][0] == ["test_slc"]
        assert df.select(pl.col("dataset(s) d'appartenance")).row(0)[0] == ["test_slc"]
        # dataset_category should be set after compute_category step, but build_from_listings doesn't set it.
...
class TestLocalLinkSlcGrd:
    ...
    def test_local_link_with_matching(self, updater):
        ...
        data = {
            ...
            "SAFE OCN": [None, None],
            "presence SLC": [None, None],
            "presence GRD": [None, None],
            "presence OCN": [None, None],
            "presence L1B XSP A21": [None, None],
            "presence L1C XSP B17": [None, None],
            "dataset(s) d'appartenance": [[], []],
            "dataset_category": [None, None],
            "Hs WW3": [None, None],
            "Tp WW3": [None, None],
            "U10 ecmwf": [None, None],
            "v10 ecmwf": [None, None],
...
class TestMergeLinkedRows:
    ...
    def test_merge_simple(self, updater):
        ...
        data = {
            ...
            "SAFE GRD": [grd, grd],
            "SAFE OCN": [None, None],
            "presence SLC": ["/path1", "/path2"],
            "presence GRD": ["/path1", "/path2"],
            "presence OCN": [None, None],
            "presence L1B XSP A21": [None, None],
            "presence L1C XSP B17": [None, None],
            "dataset(s) d'appartenance": [["ds1"], ["ds2"]],
...
class TestComputeCategoryAndConflicts:
    ...
    def test_category_priority_simple(self, updater, tmp_path):
        df = pl.DataFrame({
            ...
            "presence L1B XSP A21": [None],
            "presence L1C XSP B17": [None],
            "dataset(s) d'appartenance": [["train_ds", "val_ds"]],
            "dataset_category": [None],
            "Hs WW3": [None],
            "Tp WW3": [None],
            "U10 ecmwf": [None],
        ...
        assert result["dataset_category"][0] == "val"
    ...
    def test_category_priority_undefined(self, updater, tmp_path):
        df = pl.DataFrame({
            ...
            "presence L1B XSP A21": [None],
            "presence L1C XSP B17": [None],
            "dataset(s) d'appartenance": [["undefined_ds", "train_ds"]],
            "dataset_category": [None],
            "Hs WW3": [None],
            "Tp WW3": [None],
            "U10 ecmwf": [None],
        ...
        result = updater._compute_category_and_conflicts(df, metadata, tmp_path / "out")
        assert result["dataset_category"][0] == "train"
...
class TestMergeCatalogues:
    ...
    def test_merge_two_simple(self, tmp_path, updater):
        ...
        df2 = pl.DataFrame({
            ...
            "SAFE GRD": [None, None],
            "SAFE OCN": [None, None],
            "presence SLC": ["/path2_alt", "/path3"],
            "presence GRD": [None, None],
            "presence OCN": [None, None],
            "presence L1B XSP A21": [None, None],
            "presence L1C XSP B17": [None, None],
            "dataset(s) d'appartenance": [["ds2_new"], ["ds3"]],
            "dataset_category": ["test", "train"],
            "Hs WW3": [None, None],
            "Tp WW3": [None, None],
            "U10 ecmwf": [None, None],
            "v10 ecmwf": [None, None],
        ...
        assert sorted(slc2_row["dataset(s) d'appartenance"][0]) == ["ds2", "ds2_new"]
        # Category priority: test (3) > val (2) > train (1)
        assert slc2_row["dataset_category"][0] == "test"
...
class TestUpdatePresenceColumns:
    ...
    def test_update_presence_columns_found(self, mock_get_path, updater):
        ...
        assert result["presence SLC"][0] == "/found/path"
    ...
    def test_update_presence_columns_not_found(self, mock_get_path, updater):
        ...
        result = updater._update_presence_columns(df)
        assert result["presence SLC"][0] is None
...
class TestUpdateDerivedProducts:
    ...
    def test_update_derived_products(self, mock_get_family, updater):
        ...
        assert result["presence L1B XSP A21"][0] == "/path/to/a21"
...
class TestLinkOcnToGrd:
    ...
    def test_link_ocn_to_grd_not_found_marked(self, mock_ocn, updater):
        ...
        result = updater._link_ocn_to_grd(df)
        assert result["presence OCN"][0] == "NOT_FOUND"
