...
class TestS1IWCatalogue:
    ...
    def test_init_with_config_path(self, temp_catalogue_path, mock_config):
        ...
        with patch("s1iw_catalogue.catalogue.load_config") as mock_load_config:
            ...
            mock_load_config.assert_called_once_with(config_path=Path("/path/to/config.yml"))
    ...
    def test_write_parquet_with_metadata_success(self, catalogue_with_mocks, mock_catalogue_df, tmp_path):
    ...
    def test_write_parquet_with_metadata_fallback(self, catalogue_with_mocks, mock_catalogue_df, tmp_path):
        ...
        with patch("pyarrow.parquet.write_table") as mock_write:
            ...
            with patch.object(pl.DataFrame, "write_parquet") as mock_write_parquet:
                cat._write_parquet_with_metadata(mock_catalogue_df, test_path)
                mock_write_parquet.assert_called_once_with(test_path, compression="snappy")
    ...
    def test_create_catalogue_default_path(self, catalogue_with_mocks, mock_catalogue_df):
    ...
    def test_update_catalogue_with_merge(self, catalogue_with_mocks, mock_catalogue_df, tmp_path):
        ...
        mock_updater._compute_category_and_conflicts.return_value = existing_df  # Return existing df
    ...
    def test_update_catalogue_only_append(self, catalogue_with_mocks, mock_catalogue_df, tmp_path):
    ...
    def test_stats_all(self, catalogue_with_mocks, mock_catalogue_df, tmp_path):
        ...
        with patch("s1iw_catalogue.catalogue.CatalogueStats") as mock_stats_class:
            mock_stats = Mock()
            mock_stats.to_dict.return_value = {"total": 3, "datasets": {"SLC": 2, "GRD": 1}}
    ...
    def test_stats_with_no_data(self, catalogue_with_mocks, tmp_path):
        ...
        empty_df = pl.DataFrame({
            "SAFE SLC": [],
            "SAFE GRD": [],
            "datasets": pl.Series([], dtype=pl.List(pl.Utf8)),  # Use proper List type
            "horodating": [],
            "start date SAFE": [],
        })
    ...
    def test_stats_with_output_file(self, catalogue_with_mocks, mock_catalogue_df, tmp_path):
    ...
    def test_write_parquet_with_metadata_pyarrow_not_installed(self, catalogue_with_mocks, mock_catalogue_df, tmp_path):
